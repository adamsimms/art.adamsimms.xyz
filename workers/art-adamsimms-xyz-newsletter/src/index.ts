...
export default {
	async fetch(request: Request, env: Bindings, ctx: ExecutionContext) {
		return app.fetch(withNewsletterPrefixStripped(request), env, ctx);
	},
...
/** Serve under art.adamsimms.xyz/newsletter/* without changing route paths. */
function withNewsletterPrefixStripped(request: Request): Request {
	const url = new URL(request.url);
	if (url.pathname === "/newsletter" || url.pathname.startsWith("/newsletter/")) {
		url.pathname = url.pathname.slice("/newsletter".length) || "/";
